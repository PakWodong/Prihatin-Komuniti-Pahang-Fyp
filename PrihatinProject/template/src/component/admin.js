...
function Admin() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setIsLoading(true);
        const response1 = await fetch(`${process.env.REACT_APP_API_URL}/donation/request/`)
        ...
        const response2 = await fetch(`${process.env.REACT_APP_API_URL}/donationtransaction/add/`)
        ...
        for (let i = 6; i >= 0; i--) {
          ...
          const totalMoneyForDay = data2.reduce((total, request) => {
